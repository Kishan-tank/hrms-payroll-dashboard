...
export const getEmployeeById = async (req, res) => {
  try {
    // Also check isActive so soft-deleted employees cannot be fetched by direct ID lookup
    const employee = await Employee.findOne({ _id: req.params.id, isActive: { $ne: false } });
    if (!employee) {
...
export const deleteEmployee = async (req, res) => {
  try {
    // Soft delete by updating status to Inactive and setting soft delete flags
    const employee = await Employee.findByIdAndUpdate(req.params.id, { status: "Inactive", isActive: false, deletedAt: new Date() }, { new: true });
    if (!employee) {
      return res.status(404).json({ success: false, message: "Employee not found" });
    }
